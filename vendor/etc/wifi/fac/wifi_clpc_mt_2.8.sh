...
while(($n_check>0))
do
    wifi_status=`getprop odm.wifi.initialize.status`
    if [ "$wifi_status" = "running" ]; then
        logi "odm.wifi.initialize.status is running and n_check is :$n_check"
        n_check=$(($n_check-1))
        sleep 1
    else
        n_check=0
...
wifi_builtin=`getprop odm.wifi.driver.builtin`
